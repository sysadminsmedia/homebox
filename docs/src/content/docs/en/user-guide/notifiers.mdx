...
Generic webhooks allow Homebox to make HTTP requests to arbitrary URLs. To prevent Server-Side Request Forgery (SSRF) attacks and protect your internal network, Homebox provides several configuration options to restrict where generic notifiers can send requests. Homebox also reuses this outbound URL policy for backend integrations that fetch user-configured HTTP(S) URLs.
...
Backend integrations that fetch user-configured HTTP(S) URLs use the same outbound policy and defaults. Localhost and private networks remain reachable unless you enable `HBOX_NOTIFIER_BLOCK_LOCALHOST`, `HBOX_NOTIFIER_BLOCK_LOCAL_NETS`, `HBOX_NOTIFIER_BLOCK_NETS`, or an explicit `HBOX_NOTIFIER_ALLOW_NETS` allow list. This preserves local/LAN integration setups; tighten these options when Homebox users should not be able to reach local or private services.

### Configuration Options

These security options apply to generic webhook notifiers and backend integrations that fetch user-configured HTTP(S) URLs. Other notification services (Discord, Slack, etc.) are not affected by these settings.
...
On networks using DNS64/NAT64, hostnames resolve to IPv6 addresses with an IPv4 address embedded inside a translation prefix (RFC 6052). Without special handling, such an address would bypass all of the IPv4 rules above even though the NAT64 gateway ultimately delivers the request to the embedded IPv4 address — for example, `64:ff9b::a9fe:a9fe` is translated to `169.254.169.254`, the cloud metadata endpoint.

Homebox extracts the embedded IPv4 address from any resolved IPv6 address that falls within a configured DNS64 prefix and validates it against all of the allow/block rules above, in addition to the original IPv6 address.

```bash
HBOX_NOTIFIER_DNS64_NETS="64:ff9b::/96;64:ff9b:1::/48"
```

**Default:** `64:ff9b::/96;64:ff9b:1::/48` (the RFC 6052 well-known prefix and the RFC 8215 local-use prefix)

**Use Case:** If your network's NAT64 deployment uses a custom translation prefix instead of the well-known ones, set it here so the embedded IPv4 addresses are still validated:

```bash
HBOX_NOTIFIER_DNS64_NETS="2001:db8:64::/96"
```

**Note:** When using `HBOX_NOTIFIER_ALLOW_NETS` on a DNS64/NAT64 network, the allow list must contain both the DNS64 prefix itself and the IPv4 networks of your webhook endpoints, since every address (original and embedded) must be allowed.
