...
Notifiers can be managed programmatically via the Homebox API:

- `GET /api/v1/notifiers` - List all notifiers
- `POST /api/v1/notifiers` - Create a new notifier
- `PUT /api/v1/notifiers/{id}` - Update a notifier
- `DELETE /api/v1/notifiers/{id}` - Delete a notifier
- `POST /api/v1/notifiers/test` - Test a notifier URL

See the API documentation for detailed schemas and authentication requirements.
