...
Homebox supports sending notifications for scheduled maintenance reminders using [Shoutrrr](https://shoutrrr.nickfedor.com/), which provides integrations with many popular notification services like Discord, Slack, Telegram, and more.
...
For detailed configuration for each service, see the [Shoutrrr documentation](https://shoutrrr.nickfedor.com/latest/services/overview/).
...
Individual items can opt in to a warranty-expiration reminder by enabling **Notify 30 days before expiration** in the item's warranty details. When enabled, Homebox will:

1. Check daily for items whose warranty expires exactly 30 days from today
2. For each group with such items, collect all active notifiers
3. Send a notification listing the items whose warranty is about to expire
4. Only active notifiers will receive notifications

Like maintenance reminders, the notice fires once — on the day that is 30 days before the warranty expires. Items with a lifetime warranty or no expiry date are never included.
...
Homebox Warranties expiring on (YYYY-MM-DD):
 - Item 1
 - Item 2
```

Each section only appears when there is something to report. A notification may contain just maintenance, just warranties, or both.
...
1. **Check the URL format** - Each service has specific URL format requirements. Refer to the [Shoutrrr documentation](https://shoutrrr.nickfedor.com/latest/services/overview/).
