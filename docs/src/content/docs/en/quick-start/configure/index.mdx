...
| HBOX_SEARCH_DRIVER                      | database                                                                                       | selects the free-text search engine: `database` (searches directly in SQLite/PostgreSQL, no extra services needed) or `meilisearch`. See the [Search](./search) page.                       |
| HBOX_SEARCH_MEILISEARCH_HOST            | http://localhost:7700                                                                          | URL of the Meilisearch instance (when `HBOX_SEARCH_DRIVER=meilisearch`)                                                                                                                    |
| HBOX_SEARCH_MEILISEARCH_API_KEY         |                                                                                                | Meilisearch API key                                                                                                                                                                        |
| HBOX_SEARCH_MEILISEARCH_INDEX           | homebox_entities                                                                               | Meilisearch index uid Homebox stores entity documents in                                                                                                                                   |
| HBOX_SEARCH_MEILISEARCH_MAX_HITS        | 1000                                                                                           | maximum matches retrieved per search before database filters and pagination are applied                                                                                                    |
