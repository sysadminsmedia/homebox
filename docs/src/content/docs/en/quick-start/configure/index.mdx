...
| HBOX_SEARCH_DRIVER                      | database                                                                                       | selects the free-text search engine. `database` searches directly in SQLite/PostgreSQL (Unicode case-insensitive and accent-insensitive) and needs no extra services. The setting exists so external engines (e.g. Meilisearch) can be added in the future.       |
