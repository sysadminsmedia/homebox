...
func (ctrl *V1Controller) HandleOIDCCallback() errchain.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) error {
		...
		if err != nil {
			...
			http.Redirect(w, r, ctrl.config.Web.FrontendBase+"?oidc_error=oidc_auth_failed", http.StatusFound)
			return nil
		}

		// Set cookies and redirect to home
		ctrl.setCookies(w, noPort(r.Host), newToken.Raw, newToken.ExpiresAt, true, newToken.AttachmentToken)
		http.Redirect(w, r, ctrl.config.Web.FrontendBase+"home", http.StatusFound)
		return nil
	}
}
