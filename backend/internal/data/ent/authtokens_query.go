...
func (_q *AuthTokensQuery) prepareQuery(ctx context.Context) error {
	...
	if authtokens.Policy == nil {
		return errors.New("ent: uninitialized authtokens.Policy (forgotten import ent/runtime?)")
	}
	if err := authtokens.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *AuthTokensQuery) ForUpdate(opts ...sql.LockOption) *AuthTokensQuery {
...
func (_q *AuthTokensQuery) ForShare(opts ...sql.LockOption) *AuthTokensQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
