...
func (_q *MaintenanceEntryQuery) prepareQuery(ctx context.Context) error {
	...
	if maintenanceentry.Policy == nil {
		return errors.New("ent: uninitialized maintenanceentry.Policy (forgotten import ent/runtime?)")
	}
	if err := maintenanceentry.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *MaintenanceEntryQuery) ForUpdate(opts ...sql.LockOption) *MaintenanceEntryQuery {
...
func (_q *MaintenanceEntryQuery) ForShare(opts ...sql.LockOption) *MaintenanceEntryQuery {
	if _q.driver.Dialect() == dialect.Postgres {
		_q.Unique(false)
	}
	_q.modifiers = append(_q.modifiers, func(s *sql.Selector) {
