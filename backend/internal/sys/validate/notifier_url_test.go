...
const (
	...
	privateWebhookURL = "generic://http://192.168.1.100/webhook"
...
func TestValidateNotifierURL(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			url:  "generic://http://192.168.1.1/webhook",
			config: config.NotifierConf{
				AllowNets:      []string{privateSubnetCIDR},
				BlockLocalNets: true,
			},
			expectError: false,
		},
		...
		},
		{
			...
			url:  "generic://http://192.168.1.1/webhook",
			config: config.NotifierConf{
				BlockNets: []string{privateSubnetCIDR},
			},
			expectError: true,
		},
		{
...
func TestValidateNotifierURL_InvalidCIDR_AllowNets(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			url:  privateWebhookURL,
			config: config.NotifierConf{
				AllowNets: []string{
					"invalid-cidr",    // Invalid - should be logged and skipped
					privateSubnetCIDR, // Valid - should match
				},
			},
			expectError: false,
	...
	tests := []struct {
		...
		},
		{
			...
			config: config.NotifierConf{
				BlockNets: []string{
					"not-a-cidr",
					privateSubnetCIDR, // Valid - should block
					"also-invalid",
				},
			},
			expectError: true,
