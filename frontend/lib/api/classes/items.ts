...
export class ItemsApi extends BaseAPI {
  ...
  exportPDFURL(
    id: string,
    options: { theme?: string; photos?: boolean; owner?: string; tenant?: string } = {}
  ): string {
    ...
    return route(`/entities/${id}/export/pdf`, params);
  }
  ...
  exportAllPDFURL(options: { theme?: string; photos?: boolean; owner?: string; tenant?: string } = {}): string {
    ...
    if (options.owner) params.owner = options.owner;
    if (options.tenant) params.tenant = options.tenant;
    return route("/entities/export/pdf", params);
  }
  ...
  getPDFThemes() {
    return this.http.get<Record<string, string>>({ url: route("/entities/export/pdf/themes") });
  }
}
