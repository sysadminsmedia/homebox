<template>
  <div class="flex flex-col gap-1">
    ...
    <TagsInput
      ...
      :display-value="v => props.tags.find(l => l.id === v)?.name ?? 'Loading...'"
    >
      <div class="flex flex-wrap items-center gap-2 overflow-hidden px-3">
        <TagsInputItem v-for="item in modelValue" :key="item" :value="item" class="h-auto overflow-hidden text-wrap">
          <span
            v-if="props.tags.find(l => l.id === item)?.color"
            class="ml-2 size-4 shrink-0 rounded-full"
            :style="{ backgroundColor: props.tags.find(l => l.id === item)?.color }"
          />
              ...
              <CommandGroup>
                ...
                >
                  <span
                    ...
                    :class="{ border: props.tags.find(l => l.id === tag.value)?.color }"
                    :style="{ backgroundColor: props.tags.find(l => l.id === tag.value)?.color }"
                  />
                  {{ tag.label }}
                </CommandItem>
              </CommandGroup>
...
<script setup lang="ts">
  ...
  const filteredLabels = computed(() => {
    ...
    if (searchTerm.value.trim() !== "") {
      ...
      if (!hasExactMatch) {
        filtered.push({ value: "create-item", tag: `${t("global.create")} ${searchTerm.value}` });
      }
    }
