<script setup lang="ts">
  ...
  import type { EntitySummary, EntityTypeSummary, TagSummary } from "~~/lib/api/types/data-contracts";
  ...
  onMounted(async () => {
    ...
    await Promise.all([
      locationsStore.ensureLocationsFetched(),
    ...
    if (qEntityType) {
      selectedEntityTypes.value = entityTypes.value.filter(t => qEntityType.value.includes(t.id));
    }
  ...
  const selectedEntityTypes = ref<EntityTypeSummary[]>([]);

  const locIDs = computed(() => selectedLocations.value.map(l => l.id));
  const tagIDs = computed(() => selectedTags.value.map(l => l.id));
  const entityTypeIDs = computed(() => selectedEntityTypes.value.map(t => t.id));
  const effectiveEntityTypeIDs = computed(() => {
    if (entityTypeIDs.value.length > 0) {
      return entityTypeIDs.value;
    }

    return entityTypes.value.map(t => t.id);
  });
  ...
  async function search() {
    ...
    const push_query: Record<string, string | string[] | number | boolean | undefined> = {
      ...
      loc: locIDs.value,
      tag: tagIDs.value,
      entityTypeIds: entityTypeIDs.value,
      fields: fields,
    };
    ...
    const { data, error } = await api.items.getAll({
      ...
      entityTypeIds: effectiveEntityTypeIDs.value,
      tags: tagIDs.value,
      negateTags: negateTags.value,
      matchAllTags: matchAllTags.value,
      onlyWithoutPhoto: onlyWithoutPhoto.value,
  ...
  watchDebounced([page, pageSize, query, selectedTags, selectedLocations, selectedEntityTypes], search, {
    debounce: 250,
    maxWait: 1000,
  });
  ...
  <BaseContainer>
    <div v-if="locations && tags">
      ...
      <div class="flex w-full flex-wrap gap-2 py-2 md:flex-nowrap">
        ...
        <SearchFilter v-model="selectedEntityTypes" :label="$t('global.entity_types')" :options="entityTypes" />
        <Popover>
