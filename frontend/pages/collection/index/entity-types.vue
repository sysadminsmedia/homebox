<script setup lang="ts">
  ...
  // -----------------------
  // Create form (no isLocation checkbox)
  // -----------------------
  const createForm = reactive({
    name: "",
    icon: "",
    // fixed/unchanged in POC - you said to get rid of checkbox
    isLocation: false,
  });
  ...
  // -----------------------
  // Edit POC model (no isLocation checkbox)
  // -----------------------
  ...
  function makeRequiredGroup(): EntityTypeFieldGroup {
    return {
      ...
      fields: [
        // Immutable - names are examples; replace with your real required field list later.
        { id: "req-name", name: "Name", fieldType: "text", order: 0 },
        { id: "req-description", name: "Description", fieldType: "text", order: 1 },
        { id: "req-notes", name: "Notes", fieldType: "text", order: 2 },
        { id: "req-images", name: "Images", fieldType: "text", order: 3 },
      ],
    };
  }
  ...
  const updateForm = reactive({
    ...
    name: "",
    icon: "",
    // removed from UI, but still in payload (POC keeps existing)
    isLocation: false,
    originalIsItem: false,

    // New: optional allow-list (multiselect dropdown)
  ...
  function openEdit(et: EntityTypeSummary) {
    ...
    updateForm.allowedParentTypeIds = []; // optional for POC
    ...
    // POC: add one optional group with a couple fields
  ...
  const sortedGroups = computed(() => {
    return [...fieldGroups.value].sort((a, b) => a.order - b.order);
  });

  function reorderGroupsAfter(index: number, delta: number) {
    // required group immutable - never move it
    const groups = [...fieldGroups.value].sort((a, b) => a.order - b.order);
    if (groups[0]?.isRequiredGroup && index <= 0) return;
  ...
  function validateBeforeSave(): string | null {
    // Required group immutable - no need to validate its editing.
    const groups = [...fieldGroups.value].sort((a, b) => a.order - b.order);
  ...
  function toggleAllowedParentType(typeId: string) {
    ...
    if (idx >= 0) updateForm.allowedParentTypeIds.splice(idx, 1);
    else updateForm.allowedParentTypeIds.push(typeId);
  }
  ...
  async function update() {
    ...
    const payload = {
      ...
      fieldGroups: sortedGroups.value.map(g => ({
        id: g.id,
    ...
    // Keep existing backend update behaviour (no backend changes in POC)
    // Note: isLocation cannot be changed from UI now, but we still keep old confirm logic out.
    const { error } = await api.entityTypes.update(updateForm.id, payload as any as EntityTypeUpdate);
    if (error) {
  ...
  const parentOptions = computed(() => entityTypes.value);
  const selectedParentsSummary = computed(() => {
    if (!updateForm.allowedParentTypeIds.length) return t("global.any");
    return `${updateForm.allowedParentTypeIds.length} selected`;
  });
    ...
    <Dialog :dialog-id="DialogID.CreateEntityType">
      <DialogScrollContent>
        ...
        <form class="flex flex-col gap-3" @submit.prevent="create">
          ...
          <!-- No isLocation checkbox per request -->
    ...
    <Dialog :dialog-id="DialogID.UpdateEntityType">
      <DialogScrollContent class="max-w-4xl">
        ...
        <form class="flex flex-col gap-4" @submit.prevent="update">
          ...
          <!-- Parent restriction multiselect dropdown -->
          <div class="flex flex-col gap-2">
            ...
            <p class="text-xs text-muted-foreground">
              If left empty, any entity type can be a parent of this entity type. This would allow you to select
              multiple including this entity.
            </p>

            <Select>
              <SelectTrigger>
                <SelectValue :placeholder="selectedParentsSummary" />
              </SelectTrigger>

              <!-- POC multiselect using checkmarks inside dropdown -->
              <SelectContent class="max-h-72 overflow-auto">
                <div class="px-2 py-1">
                  <div class="text-xs font-medium text-muted-foreground">Select allowed parents</div>
                </div>
                ...
                <SelectItem
                  ...
                  @click.prevent="toggleAllowedParentType(opt.id)"
                >
                  <span class="mr-1">
                    <span v-if="updateForm.allowedParentTypeIds.includes(opt.id)">✓</span>
                    <span v-else class="opacity-40">✓</span>
                  </span>
                  <span class="truncate">{{ opt.name }}</span>
                  <span v-if="opt.isLocation" class="ml-auto text-xs text-muted-foreground"> location </span>
                </SelectItem>
                ...
                <div class="px-2 py-1">
                  <Button
                    ...
                    size="sm"
                    class="w-full"
                    :disabled="updateForm.allowedParentTypeIds.length === 0"
                    @click.prevent="updateForm.allowedParentTypeIds = []"
                  >
                    Clear restrictions
                  </Button>
                </div>
          ...
          <div class="flex flex-col gap-3">
            ...
            <p class="text-xs text-muted-foreground">
              The required group is immutable and includes built-in required fields (name, notes, images, description,
              etc). You can only configure optional groups.
            </p>

            <div class="flex flex-col gap-3">
              <Card v-for="(group, groupIndex) in sortedGroups" :key="group.id ?? group.order" class="p-3">
                <div class="flex items-center gap-3">
                  ...
                  <div class="min-w-0 flex-1">
                    <div v-if="group.isRequiredGroup" class="text-sm font-medium">Required fields (immutable)</div>
                    <div v-else class="flex items-center gap-3">
                      <Input v-model="group.name" :placeholder="`Group name`" class="h-8" />
                    </div>
                ...
                <div class="mt-3 flex flex-col gap-2">
                  <!-- Required group: immutable display -->
                  <div v-if="group.isRequiredGroup" class="flex flex-col gap-2">
                    <div
                    ...
                    >
                      <div class="flex items-center gap-2">
                        ...
                        <div class="min-w-0 flex-1">
                          <div class="truncate text-sm font-medium">{{ field.name }}</div>
                          <div class="text-xs text-muted-foreground">Type: {{ field.fieldType }}</div>
                        </div>
                      </div>
                    </div>

                    <div class="text-xs text-muted-foreground">
                      These fields are built-in required fields and cannot be edited.
                    </div>
                  </div>

                  <!-- Optional group: editable fields -->
