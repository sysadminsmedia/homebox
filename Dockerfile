...
FROM ghcr.io/mirrorshub/docker/node:18-alpine AS frontend-dependencies
WORKDIR /app
...
FROM ghcr.io/mirrorshub/docker/node:18-alpine AS frontend-builder
WORKDIR /app
...
FROM ghcr.io/mirrorshub/docker/golang:alpine AS builder-dependencies
WORKDIR /go/src/app
...
# Build API stage
FROM ghcr.io/mirrorshub/docker/golang:alpine AS builder
ARG BUILD_TIME
ARG COMMIT
ARG VERSION
